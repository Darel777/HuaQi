...
def V_calc(phi, b, k, E, t, T, dt):
    ...
    ans = gamma * np.cosh(gamma * (T - t)) / np.sinh(gamma * (T - t)) * (totv - sum) - b * 1.0 / 2 / k * E * \
          integrate.quad(f, t, T)[0]
    sum += ans * dt
...
def test():
        ...
        print (Transaction(data['code'],int(data['T']),float(data['dt']),int(data['phiindex']),int(data['Q'])))
        return str(Transaction(data['code'],int(data['T']),float(data['dt']),int(data['phiindex']),int(data['Q'])))

def Transaction(code, T, dt, phiindex, Q):
    index, res = namelist[str(code)], []
    b, k, phi, E = b_list[index], k_list[index], phi_list[phiindex], E_list[index]
    V_init(Q)
