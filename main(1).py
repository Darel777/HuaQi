...
'''
input: N, m, y_0, alpha, Lambda, q, z_0, s, e_a, e_b, prices
output: x

将这些量以全局变量的形式定义在下面，即可运行程序
'''
...
y_0 = np.array([1, 12345, 23456, 34567, 45678, 56789, 0.01, 0.03, 0.05, 0.07, 0.09, 0.11, 0.13, 0.15, 0.17, 0.19]).reshape(-1,1)
...
MAX_GENERATION = 120     #max number of generations for stopping condition
...
def main():
    ...
    print(population[0].fitness.values[0][0][0])
...
#        populationSize.append(len(population))
        ...
        # print(cnt, '/', MAX_GENERATION)
        # cnt += 1
        print("- Generation {}: Max Fitness = {}, Avg Fitness = {}".format(generationCounter, maxFitnessValue,
                                                                           meanFitnessValue))
    ...
    printMaxFV = [] 
    printMeanFV = [] 
    for item in maxFitnessValues:
        printMaxFV.append(item[0][0])
    for item in meanFitnessValues:
        printMeanFV.append(item[0][0])
    # assert(False)
    plt.plot(printMaxFV, color="red", label= 'max')
    plt.plot(printMeanFV, color="green", label = 'mean')
 #   plt.plot(populationSize, color="yellow", label = 'population')
